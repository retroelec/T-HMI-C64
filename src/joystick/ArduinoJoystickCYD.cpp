...
void ArduinoJoystickCYD::init() {
  ...
  adc_oneshot_config_channel(adc1_handle, Config::ADC_JOYSTICK_X,
                             &channel_config);
...
uint8_t ArduinoJoystickCYD::getValue() {
  ...
  // fire button
  int valueFire = gpio_get_level((gpio_num_t)Config::JOYSTICK_FIRE_PIN);
  // C64 register value
  uint8_t value = 0xff;
...
bool ArduinoJoystickCYD::getJoyOnlyModeButton() {
  return ((GPIO.in >> Config::JOYSTICK_FIRE_PIN) & 0x01) == 0;
}
#endif
