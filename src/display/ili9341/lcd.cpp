// file copied from https://github.com/witnessmenow/ESP32-Cheap-Yellow-Display
...
void Init_LCD(void) {
  ...
  lcd_init_cmd_t ili_init_cmds[] = {
      ...
      {0x26, {0x01}, 1},
      {0xE0,
       {0x1F, 0x24, 0x24, 0x0D, 0x12, 0x09, 0x52, 0XB7, 0x3F, 0x0C, 0x15, 0x06,
        0x0E, 0x08, 0x00},
       15},
      {0XE1,
       {0x00, 0x1B, 0x1B, 0x02, 0x0E, 0x06, 0x2E, 0x48, 0x3F, 0x03, 0x0A, 0x09,
        0x31, 0x37, 0x1F},
       15},
  ...
  spi_device_interface_config_t devcfg = {
      ...
      .pre_cb =
          lcd_spi_pre_transfer_callback, // 数据传输前回调，用作D/C（数据命令）线分别处理
  };
  // 初始化SPI总线
...
// 由于数据传输量很少，因此在轮询方式处理可提高速度。使用中断方式的开销要超过轮询方式。
void LCD_WriteDate(const void *data, int len) {
  esp_err_t ret;
  spi_transaction_t t;
  if (len == 0)
    return;                 // 长度为0 没有数据要传输
  ...
  t.length =
      len *
      8; // 要写入的数据长度 Len 是字节数，len, transaction length is in bits.
...
void LCD_Restore_Interface() {
  if (LCD_SPI_Handle != NULL) {
    spi_bus_remove_device(LCD_SPI_Handle);
    LCD_SPI_Handle = NULL;
  }
  spi_device_interface_config_t devcfg = {
      .mode = 0,
  ...
  esp_err_t ret = spi_bus_add_device(LCD_HOST, &devcfg, &LCD_SPI_Handle);
  ESP_ERROR_CHECK(ret);
}
